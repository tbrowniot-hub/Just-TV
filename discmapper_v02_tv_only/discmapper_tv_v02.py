...
def compute_typical_runtime_seconds(files: List[Dict[str, Any]], episode_count: int) -> Optional[int]:
    ...
    if not durs:
        return None
    take_n = min(len(durs), max(1, episode_count))
    sample = durs[:take_n]
    try:
...
def dp_map_files_to_episodes(
    ...
    pairs.reverse()
    errs = [abs(files[fj]["duration_s"] - typical_s) / 60.0 for ei, fj in pairs]
...
def tv_dest_paths(ready_root: Path, series: str, show_year: Optional[int], include_year: bool, season: int, sxxeyy: str, ep_title: str) -> Tuple[Path, Path]:
...
def cmd_queue_builder(args: argparse.Namespace) -> None:
    ...
    root = tk.Tk()
    root.title("DiscMapper TV v0.2 — Build Rip Queue (Discs)")
...
def cmd_rip_queue(args: argparse.Namespace) -> None:
    ...
    for pos, key in enumerate(keys, start=1):
        ...
        print(f"[DiscMapper TV] Dirty mode: ripping ALL titles. minlength={floor_s}s")
        rc = makemkv_rip_all(makemkv, drive_index, job_dir, floor_s, rip_log)
        if rc != 0:
        ...
        for p in mkvs:
            ...
            if len(files) < len(eps):
                print(f"[DiscMapper TV] Not enough episode-length files ({len(files)} found for {len(eps)} eps) → Review")
                shutil.move(str(job_dir), str(review_root / job_name))
                continue
            ...
            print(f"[DiscMapper TV] SUCCESS: moved {moved} eps → {ready_root}\\{show_folder_name(series, show_year, include_year)}\\Season {season:02d}")
